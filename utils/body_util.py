...
def get_global_RTs(cnl_gtfms, dst_Rs, dst_Ts, use_smplx=False, cnl_gtfms_inverse=False):
    if not use_smplx:
        PARENT = SMPL_PARENT
    else:
        PARENT = SMPLX_PARENT
    ...
    dst_gtfms = [local_Gs[:, 0, :, :]]

    for i in range(1, total_bones):
        dst_gtfms.append(torch.matmul(
            dst_gtfms[PARENT[i]],
            local_Gs[:, i, :, :]))
    dst_gtfms = torch.stack(dst_gtfms, dim=1)
    ...
    if not cnl_gtfms_inverse:
        f_mtx = torch.matmul(dst_gtfms, torch.inverse(cnl_gtfms))
    else:
        f_mtx = torch.matmul(dst_gtfms, cnl_gtfms)
...
def get_joints(dst_Rs, dst_Ts, use_smplx=False):
    if not use_smplx:
        PARENT = SMPL_PARENT
    else:
        PARENT = SMPLX_PARENT

    total_bones = dst_Ts.shape[-2]
    ...
    for i in range(1, total_bones):
        dst_gtfms.append(torch.matmul(
            ...
            local_Gs[:, i, :, :]))
    dst_gtfms = torch.stack(dst_gtfms, dim=1)
    dst_gtfms = dst_gtfms.view(dst_Ts.shape[0], -1, 4, 4)

    return dst_gtfms[:, :, :3, 3]
...
def apply_lbs(xyzs_canonical, global_Rs, global_Ts, lbs_weights):
    ...
    B x N x 3
    """
    xyzs_trans = torch.matmul(global_Rs[:, None], xyzs_canonical[:, :, None, :, None]).squeeze(-1) + global_Ts[:, None] # B x N x J x 3
    xyzs_sum = torch.sum(xyzs_trans * lbs_weights.unsqueeze(-1), dim=-2)
    return xyzs_sum
...
def body_RTs_to_global_RTs(Rs, Ts, use_smplx=False):
    if not use_smplx:
        PARENT = SMPL_PARENT
    else:
        PARENT = SMPLX_PARENT

    global_trans = np.eye(4)[None, :, :].repeat(Rs.shape[0], axis=0)
    global_trans[0, :3, :3] = Rs[0]
    global_trans[0, :3, 3] = Ts[0]

    for i, (R, T) in enumerate(zip(Rs[1:], Ts[1:])):
        global_trans[i + 1, :3, :3] = R
        global_trans[i + 1, :3, 3] = T
        global_trans[i + 1] = global_trans[PARENT[i + 1]] @ global_trans[i + 1]
    return global_trans[:, :3, :3], global_trans[:, :3, 3]
