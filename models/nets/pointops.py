...
class FurthestSampling(Function):
    @staticmethod
    def forward(ctx, xyz, offset, new_offset, stride):
        """
        input: xyz: (n, 3), offset: (b), new_offset: (b)
        output: idx: (m)
        """
        assert xyz.is_contiguous()
        n, b = xyz.shape[0], offset.shape[0]
        # n, b, n_max = xyz.shape[0], offset.shape[0], offset[0]
        # for i in range(1, b):
        #     n_max = max(offset[i] - offset[i-1], n_max)
        n_max = torch.max(torch.cat([offset[0:1], torch.diff(offset)]))
        # idx = torch.cuda.IntTensor(new_offset[b-1].item()).zero_()
        idx = torch.cuda.IntTensor(n // stride).zero_()
        tmp = torch.cuda.FloatTensor(n).fill_(1e10)
        pointops_cuda.furthestsampling_cuda(b, n // b, xyz, offset, new_offset, tmp, idx)
        del tmp
        return idx
