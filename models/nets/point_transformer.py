...
class TransitionDown(nn.Module):
    ...
    def forward(self, pxo):
        ...
        if self.stride != 1:
            ...
            n_o = torch.cat([o[0:1], torch.diff(o)]) // self.stride
            idx = pointops.furthestsampling(p, o, n_o, self.stride)  # (m)
            n_p = p[idx.long(), :]  # (m, 3)
...
class TransitionUp(nn.Module):
    ...
    def forward(self, pxo1, pxo2=None):
        if pxo2 is None:
            ...
            #     x_tmp.append(x_b)
            # x_tmp = torch.cat(x_tmp, 0)

            x = x.reshape(b, n // b, -1)
            x = torch.cat([x, self.linear2(x.sum(1, True) / cnt).repeat(1, cnt, 1)], dim=-1)
            x = x.reshape(n, -1)
...
class PointTransformerEnc(nn.Module):
    def __init__(self, block, blocks, c=6, freeze_bn=True):
        super().__init__()
    ...
    def load_pretrained_weight(self, weigth_path: str) -> None:
        ...
        static_dict.pop('enc1.0.linear.weight') # since input dim not matched
